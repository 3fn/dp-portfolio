...
var canvas = document.getElementById("career-chart");
if (canvas) {
  let build3fn = function() {
    const segs = [];
    ...
    return PAD.left + (y - YEAR_START) / (YEAR_END - YEAR_START) * (W - PAD.left - PAD.right);
  }, easeOut = function(t) {
    ...
    ctx.strokeStyle = "rgba(178,188,196,0.15)";
    ctx.lineWidth = 0.5;
    ...
    ctx.fillStyle = "rgba(38,50,58,0.5)";
    ctx.font = "10px ui-monospace,monospace";
    ctx.textAlign = "center";
    for (let y = 2006; y <= 2026; y += 2) ctx.fillText(String(y), xFor(y), H - PAD.bottom + 16);
    ctx.fillStyle = "rgba(38,50,58,0.35)";
    ctx.font = "9px ui-monospace,monospace";
    ctx.textAlign = "left";
    ctx.fillText("Design", PAD.left, PAD.top - 12);
    ctx.fillText("Engineering", PAD.left, H - PAD.bottom - 6);
    for (const seg of ALL) {
      ...
      if (seg.design > 0) {
        ...
        ctx.fillStyle = g;
        ctx.fillRect(segX1, mid - bh, segW, bh);
      ...
      if (seg.eng > 0) {
        ...
        ctx.fillStyle = g;
        ctx.fillRect(segX1, mid, segW, bh);
        ctx.fillStyle = noisePattern;
        ctx.fillRect(segX1, mid, segW, bh);
      }
      ...
      if (seg.design > 0) {
        ...
        ctx.beginPath();
        ctx.moveTo(nx, mid);
        ctx.lineTo(nx, mid - bh);
        ctx.strokeStyle = lineColorDesign;
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(segX1, mid - bh);
        ctx.lineTo(segX1 + segW, mid - bh);
        ctx.strokeStyle = lineColorDesign;
        ctx.lineWidth = 2;
        ctx.stroke();
      }
    ...
    ctx.lineTo(W - PAD.right, mid);
    ctx.strokeStyle = "rgba(34,34,42,1)";
    ctx.lineWidth = 1.5;
    ctx.stroke();
    for (const seg of ALL) {
      ...
      ctx.beginPath();
      ctx.arc(nx, mid, isHov ? 5 : 3.5, 0, Math.PI * 2);
      ...
      ctx.font = `${isHov ? "bold " : ""}9px ui-monospace,monospace`;
      ctx.textAlign = "center";
      ctx.fillText(seg.short, nx, mid + half * 0.82);
    }
    if (animT < 1 || hovered !== null) requestAnimationFrame(draw);
    else animDone = true;
  };
  build3fn2 = build3fn, setup2 = setup, xFor2 = xFor, easeOut2 = easeOut, draw2 = draw;
  const ctx = canvas.getContext("2d");
  const tooltip = document.getElementById("career-tooltip");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  let W, H, DPR, hovered = null;
  let animT = 0, lastTs = null, animDone = false;
  const PAD = { top: 40, bottom: 44, left: 16, right: 16 };
  const YEAR_START = 2005.6, YEAR_END = 2026.5, CAREER_START = 2006;
  const ALL_ROLES = [
    ...
    { id: "dp", label: "DesignerPunk", short: "DP", yearStart: 2025, yearEnd: 2026.3, design: 45, eng: 55, period: "2025 \u2013 Present", desc: "True Native cross-platform design system. 310+ tokens, 30 components, 8k+ tests, MCP servers, 9 custom AI agents.", is3fn: false, yearMid: 0 }
  ];
  const TFN = 60, TFN_E = 40;
  const ALL = [...build3fn(), ...ALL_ROLES].sort((a, b) => a.yearStart - b.yearStart);
  ALL.forEach((s) => {
    s.yearMid = (s.yearStart + s.yearEnd) / 2;
  });
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    for (const seg of ALL) {
      ...
      if (mx2 > sx1 - 4 && mx2 < sx2 + 4 && my2 > mid - dh - 8 && my2 < mid + eh + 8) {
        found = seg;
        break;
      }
    }

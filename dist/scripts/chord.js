...
if (canvas) {
  let resize = function() {
    ...
    return a + (b - a) * t;
  }, buildLayout = function(rot) {
    const shortSide = Math.min(W(), H());
    const outerR = shortSide * 0.4, innerR = shortSide * 0.245;
  ...
  }, drawArcBand = function(startA, endA, R, color, label, isInner) {
    const thick = isInner ? 7 : 11;
    ctx.beginPath();
    ctx.arc(CX(), CY(), R + thick / 2, startA, endA);
    ctx.strokeStyle = color;
    ctx.lineWidth = thick;
    ctx.globalAlpha = isInner ? 0.12 : 0.15;
    ctx.stroke();
    ctx.globalAlpha = 1;
    ctx.beginPath();
    ctx.arc(CX(), CY(), R + thick, startA, endA);
    ctx.strokeStyle = color;
    ctx.lineWidth = 1;
    ctx.globalAlpha = 0.6;
    ctx.stroke();
    ...
    ctx.restore();
  }, drawChord = function(a, b, w, col, alpha, pulse) {
    ...
    ctx.fillStyle = PAL.root;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("DESIGNER", CX(), CY() - 7);
    ctx.fillText("PUNK", CX(), CY() + 7);
  }, frame = function(ts) {
    if (!reducedMotion) {
      pulses.forEach((p) => {
        p.t += p.speed;
    ...
    allNodes.forEach((n) => {
      if (n.x && Math.hypot(mx - n.x, my - n.y) < 16) nodeUnderMouse = n;
    });
    ...
    hoveredNode = null;
    allNodes.forEach((n) => {
      if (n.x && Math.hypot(mx - n.x, my - n.y) < 16) hoveredNode = n;
    });
    ...
    CONNECTIONS.forEach(([aid, bid, w], i) => {
      ...
      drawChord(a, b, w, col, active ? 0.62 : 0.1, pulses[i]);
    });
    allNodes.forEach((n) => {
      if (n.x) drawNode(n, hoveredNode?.id === n.id);
    });
    drawRoot();
  ...
  resize2 = resize, hexA2 = hexA, lerp2 = lerp, buildLayout2 = buildLayout, drawArcBand2 = drawArcBand, drawChord2 = drawChord, drawNode2 = drawNode, drawRoot2 = drawRoot, frame2 = frame;
  ...
  const OUTER_GROUPS = [
    { id: "mcp", label: "MCP SERVERS", color: "#00aabb", startAngle: 0, endAngle: 0, nodes: [
      { id: "docs-mcp", label: "Docs MCP", ck: "mcp", size: 2, desc: "82% token reduction \xB7 progressive disclosure", ring: "outer", group: "mcp", angle: 0, x: 0, y: 0, R: 0 },
      { id: "app-mcp", label: "Application MCP", ck: "mcp", size: 2, desc: "10 query tools \xB7 agent-driven component selection", ring: "outer", group: "mcp", angle: 0, x: 0, y: 0, R: 0 },
      { id: "product-mcp", label: "Product MCP", ck: "mcp", size: 2, desc: "YAML context layer \xB7 directs agents toward goals", ring: "outer", group: "mcp", angle: 0, x: 0, y: 0, R: 0 },
      { id: "figma-mcp", label: "Figma Console MCP", ck: "figma", size: 1.5, desc: "Bidirectional Figma \xB7 token push", ring: "outer", group: "mcp", angle: 0, x: 0, y: 0, R: 0 }
    ] },
    { id: "agents", label: "AGENTS", color: "#ff2d8f", startAngle: 0, endAngle: 0, nodes: [
      { id: "ada", label: "Ada", ck: "agent", size: 1.2, desc: "Token development \xB7 Rosetta compliance", ring: "outer", group: "agents", angle: 0, x: 0, y: 0, R: 0 },
      { id: "lina", label: "Lina", ck: "agent", size: 1.2, desc: "Component development \xB7 Stemma compliance", ring: "outer", group: "agents", angle: 0, x: 0, y: 0, R: 0 },
      { id: "thurgood", label: "Thurgood", ck: "agent", size: 1.2, desc: "System auditing \xB7 governance", ring: "outer", group: "agents", angle: 0, x: 0, y: 0, R: 0 },
      ...
      { id: "stacy", label: "Stacy", ck: "agent", size: 1.2, desc: "Product governance \xB7 lessons-learned", ring: "outer", group: "agents", angle: 0, x: 0, y: 0, R: 0 },
    ...
    { id: "systems", label: "SYSTEMS", color: "#7a00cc", startAngle: 0, endAngle: 0, nodes: [
      ...
      { id: "stemma", label: "Stemma", ck: "stemma", size: 2.5, desc: "Component architecture \xB7 contracts", ring: "outer", group: "systems", angle: 0, x: 0, y: 0, R: 0 },
  ...
  const INNER_GROUPS = [
    { id: "tokens", label: "TOKENS", color: "#1a5fff", startAngle: 0, endAngle: 0, nodes: [
      { id: "primitive", label: "Primitive tokens", ck: "rosetta", size: 1.2, desc: "space \xB7 color \xB7 typography", ring: "inner", group: "tokens", angle: 0, x: 0, y: 0, R: 0 },
      { id: "semantic", label: "Semantic tokens", ck: "rosetta", size: 1.2, desc: "color.primary \xB7 space.inset.*", ring: "inner", group: "tokens", angle: 0, x: 0, y: 0, R: 0 },
    ...
    { id: "components", label: "COMPONENTS", color: "#9933ff", startAngle: 0, endAngle: 0, nodes: [
      ...
      { id: "contracts", label: "Contracts", ck: "stemma", size: 1.5, desc: "210 concepts \xB7 10 categories", ring: "inner", group: "components", angle: 0, x: 0, y: 0, R: 0 }
    ] }
  ];
  const nodeMap = {};
  OUTER_GROUPS.forEach((g) => g.nodes.forEach((n) => {
    nodeMap[n.id] = n;
  }));
  INNER_GROUPS.forEach((g) => g.nodes.forEach((n) => {
    nodeMap[n.id] = n;
  }));
  const CONNECTIONS = [
    ["docs-mcp", "rosetta", 0.8],
    ["docs-mcp", "stemma", 0.5],
    ["docs-mcp", "ada", 0.7],
    ["docs-mcp", "lina", 0.7],
    ["app-mcp", "stemma", 0.9],
    ["app-mcp", "sparky", 0.6],
    ["app-mcp", "leonardo", 0.5],
    ["product-mcp", "leonardo", 0.8],
    ["product-mcp", "stacy", 0.7],
    ["product-mcp", "sparky", 0.5],
    ["figma-mcp", "dtcg", 0.8],
    ["figma-mcp", "figma-out", 0.9],
    ["rosetta", "primitive", 0.9],
    ["rosetta", "semantic", 0.9],
    ["rosetta", "dtcg", 0.7],
    ["stemma", "prim-comp", 0.9],
    ["stemma", "sem-comp", 0.9],
    ["stemma", "contracts", 0.9],
    ["primitive", "semantic", 0.6],
    ["prim-comp", "sem-comp", 0.6],
    ["ada", "rosetta", 0.7],
    ["lina", "stemma", 0.7],
    ["a2ui", "rosetta", 0.5],
    ["semantic", "prim-comp", 0.4],
    ["stacy", "product-mcp", 0.5]
  ];
  const pulses = CONNECTIONS.map(() => ({ t: Math.random(), speed: 18e-4 + Math.random() * 2e-3 }));
  let rotOffset = 0, hoverFade = 0, hoveredNode = null;
  let dragging = false, lastDragAngle = 0, spinVelocity = 4e-4;
  const IDLE_SPEED = 4e-4, HOVER_DECEL = 0.07, RESUME_ACCEL = 0.03, GROUP_GAP = 0.1;

...
var STATES = {
  ...
  "beat-principle": {
    nodes: [
      { x: 80, y: 100, r: 8, color: "#80F6FF", label: "200", tip: "space200: base \xD7 2 = 16.\nFormula-based origin." },
      { x: 160, y: 100, r: 8, color: "#80F6FF", label: "300", tip: "space300: base \xD7 3 = 24.\nFormula-based origin." },
...
var BEAT_IDS = ["beat-problem", "beat-principle", "beat-architecture", "beat-payoff"];
function init() {
  const svg = document.querySelector(".viz-svg");
  const nodesG = svg?.querySelector(".viz-nodes");
  const connsG = svg?.querySelector(".viz-connections");
  ...
  const sections = BEAT_IDS.map((id) => document.getElementById(id)).filter(Boolean);
  ...
  const onScroll = () => {
    if (activeNode !== null) hideTooltip();
  };
